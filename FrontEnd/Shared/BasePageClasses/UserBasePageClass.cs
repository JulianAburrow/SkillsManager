...
public class UserBasePageClass : BasePageClass
{
    ...
    protected void CopyDisplayModelToModel()
    {
        ...
        for (var i = 0; i < UserDisplayModel.UserSkill.Count; i++)
        {
            ...
            {
                SkillId = UserDisplayModel.UserSkill[i].SkillId,
                SkillScore = UserDisplayModel.UserSkill[i].SkillScore,
            });
            //UserModel.UserSkill[i].SkillId = UserDisplayModel.UserSkill[i].SkillId;
            //UserModel.UserSkill[i].SkillScore = UserDisplayModel.UserSkill[i].SkillScore;
        }
    }
}
